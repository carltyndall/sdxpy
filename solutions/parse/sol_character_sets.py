"""Solution: Character Sets exercise.

Modify the parser so that expressions like [xyz] are interpreted to mean
"match any one of those three characters" — a shorthand for {x,y,z}.
"""
...
class Tokenizer:
    """Tokenizer extended to handle [ and ] for character sets."""
...
# --- Match classes (the AST nodes, copied from the chapter) ---
...
# --- Parser extended for character sets ---
...
class Parser:
    ...
    def _parse_CharsetStart(self, rest, back):
        """Gather Lit tokens until CharsetEnd, then build an Either."""
        children = []
...
if __name__ == "__main__":
    ...
    # [xyz] is equivalent to {x,y,z}
    result = p.parse("[xyz]")
    print("Input:    [xyz]")
    print("Result:  ", result)
    # Expected: Either([Lit('x'), Lit('y'), Lit('z')])

    # Mixed with other patterns
    result = p.parse("*[abc]")
    print("Input:    *[abc]")
    print("Result:  ", result)
    # Expected: Any(Either([Lit('a'), Lit('b'), Lit('c')]))

    # Inside a larger literal context
    result = p.parse("hello[12]")
    print("Input:    hello[12]")
    print("Result:  ", result)
    # Expected: Lit('hello', Either([Lit('1'), Lit('2')]))
